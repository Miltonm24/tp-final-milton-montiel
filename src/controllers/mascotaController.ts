...
import { CreateMascotaDTO, UpdateMascotaDTO } from '../dtos/mascota.dto.js';
...
export const crearMascota = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { nombre, especie, raza, edad, duenoId }: CreateMascotaDTO = req.body;
...
export const actualizarMascota = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const datosActualizar: UpdateMascotaDTO = req.body;
